...
import { deleteAnime, edit, getAnimeDetails } from '../services/animeService.js'
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        if(response) setNetworkAnime(response)
      } catch (error) {
        ...
        <div className="flex items-end space-x-5">
          ...
          {isAuthenticated && inList && changes && (
            <button 
              disabled={isSaving}
              onClick={handleSave}
              className={`ml-auto shrink-0 px-4 py-2 bg-[#A77510]/20 border border-[#A77510]/50 rounded-xl text-[10px] font-black tracking-widest text-[#E6BD9E] transition-all duration-300 shadow-lg shadow-[#A77510]/5 flex items-center gap-2 ${
