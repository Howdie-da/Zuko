...
function Discover({
  ...
  useEffect(() => {
    ...
    const fetchDiscoverFeeds = async () => {
      try {
        ...
        dispatch(setDiscoverData(response.data.data))
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      try {
        const response = await apiClient.get(`/anime/search?q=${encodeURIComponent(search)}`)
        if(response.data?.data) setSearchResults(response.data.data)
      } catch (error) {
